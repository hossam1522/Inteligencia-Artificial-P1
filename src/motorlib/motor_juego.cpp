...
bool actuacionJugador(unsigned char celda_inicial, unsigned char celda_fin, Action accion, unsigned int x, unsigned int y)
{
  ...
  case actTURN_SL:
    ...
    monitor.girar45JugadorIzquierda();
    salida = true;
    if (celda_inicial == 'X')
    { // Casilla Rosa (Recarga)
      monitor.get_entidad(0)->increaseBateria(10);
  ...
  case actTURN_BR:
    monitor.get_entidad(0)->giro135Dch();
    monitor.girar135JugadorDerecha();
    salida = true;
    if (celda_inicial == 'X')
    { // Casilla Rosa (Recarga)
      monitor.get_entidad(0)->increaseBateria(10);
  ...
  case actTURN_BL:
    monitor.get_entidad(0)->giro135Izq();
    monitor.girar135JugadorIzquierda();
    salida = true;
    if (celda_inicial == 'X')
    { // Casilla Rosa (Recarga)
      monitor.get_entidad(0)->increaseBateria(10);
...
bool actuacionNPC(unsigned int entidad, unsigned char celda, Action accion, unsigned int x, unsigned int y)
{
  ...
  case lobo:    // Acciones para el lobo
    ...
    case actTURN_BR:
      monitor.get_entidad(entidad)->giro135Dch();
      out = true;
      break;

    case actTURN_BL:
      monitor.get_entidad(entidad)->giro135Izq();
      out = true;
      break;
    }
  }
