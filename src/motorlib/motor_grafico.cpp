...
void botonElegirMapaCB(int valor)
{
  ...
  panelSelecMapa->add_radiobutton_to_group(group, "Nivel 2: Con aldeanos                    ");
  panelSelecMapa->add_radiobutton_to_group(group, "Nivel 3: Con lobos                       ");
...
void lanzar_motor_grafico(int argc, char **argv)
{
  ...
  // Main Window
  ventanaPrincipal = glutCreateWindow("Practica 1: Agentes Reactivos. Curso 22/23");
...
void lanzar_motor_grafico_verOnline(int argc, char **argv, EnLinea &argumentos)
{
  ...
  // Main Window
  ventanaPrincipal = glutCreateWindow("Practica 1: Agentes Reactivos (22/23). Version BAJO PARAMETROS");
  // Main Window callback function
  glutReshapeFunc(reshape);
  glutDisplayFunc(display_ventPrincipal);
